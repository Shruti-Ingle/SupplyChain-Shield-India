...
function MatchesContent() {
  ...
  return (
    <div>
      {ToastComponent}
      <h1 className="text-2xl font-bold mb-2">AI-Generated Matches</h1>
      <p className="text-gray-500 mb-6">Best-fit loads for your empty return trips</p>

      {matches.length === 0 ? (
        <div className="card text-center py-12">
          <p className="text-gray-500">No matches found. Post an available route to get AI suggestions.</p>
        </div>
      ) : (
            ...
            <div key={m.id} className="card">
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500">Customer: {m.business_name}</p>
                  <div className="flex flex-wrap gap-4 mt-3 text-sm">
                    ...
                    <span className="flex items-center gap-1 text-india-green"><Leaf size={14} /> {m.co2_saved} kg CO₂</span>
                  </div>
                </div>
      ...
      {confirmId && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl p-6 max-w-sm w-full">
            ...
            <p className="text-gray-600 text-sm mb-4">Accept this shipment? It will become an active trip and be removed from other transporters.</p>
